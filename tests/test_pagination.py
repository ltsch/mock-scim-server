...
class TestPagination:
    ...
    def test_pagination_start_index_mapping(self, client):
        ...
        response = client.get("/v2/Users/?startIndex=3&count=2", headers=self.AUTH_HEADERS)
        assert response.status_code == 200
        data = response.json()
        ...
        first_page_users = set(user['userName'] for user in client.get("/v2/Users/?startIndex=1&count=2", headers=self.AUTH_HEADERS).json()['Resources'])
        second_page_users = set(user['userName'] for user in client.get("/v2/Users/?startIndex=3&count=2", headers=self.AUTH_HEADERS).json()['Resources'])
    ...
    def test_pagination_consistency(self, client):
        ...
        page = 1
        while True:
            response = client.get(f"/v2/Users/?startIndex={page}&count=2", headers=self.AUTH_HEADERS)
            assert response.status_code == 200
            data = response.json()
    ...
    def test_pagination_edge_cases(self, client):
        ...
        response = client.get("/v2/Users/?startIndex=1&count=1", headers=self.AUTH_HEADERS)
        assert response.status_code == 200
        data = response.json()
        assert data['startIndex'] == 1
        ...
        # Test middle page
        response = client.get("/v2/Users/?startIndex=5&count=1", headers=self.AUTH_HEADERS)
        assert response.status_code == 200
        data = response.json()
        ...
        response = client.get("/v2/Users/?startIndex=999&count=1", headers=self.AUTH_HEADERS)
        assert response.status_code == 200
        data = response.json()
    ...
    def test_pagination_with_filtering(self, client):
        ...
        response = client.get("/v2/Users/?startIndex=1&count=2&filter=displayName%20co%20%22John%22", headers=self.AUTH_HEADERS)
        assert response.status_code == 200
        data = response.json()
        assert data['startIndex'] == 1
    ...
    def test_all_resource_types_pagination(self, client):
        ...
        resource_types = ['Users', 'Groups', 'Entitlements', 'Roles']
        
        for resource_type in resource_types:
            # Test first page
            response = client.get(f"/v2/{resource_type}/?startIndex=1&count=2", headers=self.AUTH_HEADERS)
            assert response.status_code == 200
            data = response.json()
            assert data['startIndex'] == 1
            assert data['itemsPerPage'] <= 2
            ...
            if data['totalResults'] > 2:
                response = client.get(f"/v2/{resource_type}/?startIndex=3&count=2", headers=self.AUTH_HEADERS)
                assert response.status_code == 200
                data = response.json()
